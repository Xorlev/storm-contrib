 ...
 */
public abstract class HbaseIncrementBolt extends HbaseBolt {
    private final ConcurrentHashMap<MessageId, Tuple> _pendingWrites = new ConcurrentHashMap<MessageId, Tuple>();

    protected HbaseIncrementBolt(String table, String quorumSpec) {
        super(table, quorumSpec);
    }

    protected HbaseIncrementBolt(byte[] table, String quorumSpec) {
        super(table, quorumSpec);
    }
